...
if "boot_done" not in st.session_state:
    ...
    try:
        with get_engine().connect() as c:
            print("DB OK:", c.exec_driver_sql("select version()").scalar())
...
# ========= utilities =========
def qp_get_all() -> dict:
    """Query params as {key: single string} (new & legacy Streamlit)."""
    try:
        qp = st.query_params
    except Exception:
...
if "PAGE_MAP" not in st.session_state:
    ...
    st.session_state.PAGE_MAP = {
        ...
        "Admin • All Designs": pa_lib.render_admin_library,  # will be hidden unless superadmin
    }
