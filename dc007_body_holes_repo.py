# dc007_body_holes_repo.py — with audit logging like dc001_repo.py
...
TABLE  = "dc007_body_holes_calcs"
ENTITY = "dc007_body_holes"

# -------------------------------------------------------------------
# Create
# -------------------------------------------------------------------
def create_dc007_body_holes_calc(
    user_id: str,
    name: str,
    ...
    """
    Insert a DC007-2 (Body holes) calculation. `payload` is a Python dict (JSONB).
    If `design_id` is provided, the row is linked to a valve design.
    Returns the new row id (as text/uuid).
    """
    clean_name = (name or "DC007-Body-Holes").strip() or "DC007-Body-Holes"
    sql = f"""
    ...
    with connect() as conn:
        ...
        # AUDIT (store a compact summary from payload["base"] if available)
        try:
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc007_body_holes_calcs(user_id: str, limit: int = 200):
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc007_body_holes_calc(calc_id: str, user_id: str) -> dict | None:
    with connect() as conn:
        r = conn.execute(
            text(f"""
...
# -------------------------------------------------------------------
def update_dc007_body_holes_calc(
    calc_id: str,
    user_id: str,
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC007-Body-Holes").strip() or "DC007-Body-Holes"
    ...
    """
    with connect() as conn:
        # Read old name first so we always have something for the log
        old = conn.execute(
...
def delete_dc007_body_holes_calc(calc_id: str, user_id: str) -> bool:
    sql = f"DELETE FROM {TABLE} WHERE id = :id AND user_id = :uid"
    with connect() as conn:
        # Read the name BEFORE deleting to preserve it in audit logs
        try:
