...
def render_dc006():
    ...
    Symbols (units):
      Pa  [MPa]  design pressure (MPa = N/mm²)
      Bcd [mm]   bolt circle diameter
      FT  [mm]   flange thickness
      ESGD, ISGD [mm] external / internal seal gasket diameters
      m [-], y [MPa]  gasket factors

    Derived (per your sheet):
      N   = (ESGD − ISGD) / 2
      b0  = N/2
      b   = b0          (your sheet uses b = 2.20 = N/2)
      G   = ESGD − 2b
      H   = (π/4) · G² · Pa
      Hp  = 2 · b · π · G · m · Pa
      Wm1 = H + Hp
      ...
      Sf2 = K · Wm2 / FT²
      Sf  = max(Sf1, Sf2)
      Check: Sf ≤ Allowable (material table)
    """
    ...
    # ---- Inputs block (green in sheet)
    Pa_default = float(st.session_state.get("operating_pressure_mpa", 10.21))

    c1, c2, c3 = st.columns([1.1,1.1,1.1])
    with c1:
        Pa  = st.number_input("Design pressure at ambient temperature Pa [MPa]", value=Pa_default, step=0.01, format="%.2f")
    ...
    with c2:
        ...
        gasket = st.selectbox("Gasket type", options=list(GASKETS.keys()), index=list(GASKETS.keys()).index("GRAPHITE"))
    with c3:
    ...
    # Widths and reaction diameter
    N  = (ESGD - ISGD) / 2.0
    b0 = N / 2.0
    b  = b0                        # as on your sheet
    G  = ESGD - 2.0 * b
    ...
    with o1: st.text_input("Hydrostatic end force H [N] = π/4 × G² × Pa", value=f"{H:,.2f}", disabled=True)
    with o2: st.text_input("Joint contact surface compression load Hp [N] = 2 × b × π × G × m × Pa", value=f"{Hp:,.2f}", disabled=True)
    with o3: st.text_input("Min. req. bolt load at ambient temperature Wm1 [N] = H + Hp", value=f"{Wm1:,.2f}", disabled=True)
    ...
    with s1: st.text_input("Operating condition at ambient temperature Sf₁ [MPa] = 2/π·(1−0.67·ESGD/Bcd)·Wm1/FT²", value=f"{Sf1:.2f}", disabled=True)
    ...
    # One row material (your sheet shows ASTM A350 LF2 CL.1 with 161 MPa)
    ...
    # Persist for downstream use or export
