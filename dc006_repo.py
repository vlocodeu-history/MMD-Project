# dc006_repo.py — with audit logging like dc001_repo.py
...
def create_dc006_calc(
    ...
    """
    Insert a DC006 calculation. `payload` is a Python dict (JSONB).
    If `design_id` is provided, the row is linked to a valve design.
    Returns the new row id (as text/uuid).
    """
    clean_name = (name or "DC006").strip() or "DC006"
    sql = f"""
    ...
    with connect() as conn:
        ...
        # AUDIT (compact summary pulled from payload["base"] if present)
        try:
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc006_calcs(user_id: str, limit: int = 200):
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc006_calc(calc_id: str, user_id: str) -> dict | None:
    with connect() as conn:
        r = conn.execute(
            text(f"""
...
def update_dc006_calc(
    ...
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC006").strip() or "DC006"
    ...
    """
    with connect() as conn:
        # Grab old name so we always have something to show in audit
        old = conn.execute(
...
def delete_dc006_calc(calc_id: str, user_id: str) -> bool:
