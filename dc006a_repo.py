# dc006a_repo.py — with audit logging like dc001_repo.py
...
def create_dc006a_calc(
    ...
    """
    Insert a DC006A calculation. `payload` is a Python dict (JSONB).
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc006a_calcs(user_id: str, limit: int = 200):
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc006a_calc(calc_id: str, user_id: str) -> dict | None:
    with connect() as conn:
        r = conn.execute(
            text(f"""
...
def update_dc006a_calc(
    ...
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC006A").strip() or "DC006A"
