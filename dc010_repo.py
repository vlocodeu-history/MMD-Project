# dc010_repo.py — with audit logging like dc001_repo.py
...
def create_dc010_calc(
    ...
    """
    Insert a DC010 calculation. `payload` is a Python dict (JSONB).
    If `design_id` is provided, this row is linked to a valve design.
    Returns the new row id (uuid::text).
    """
    clean_name = (name or "DC010").strip() or "DC010"
    sql = f"""
    ...
    with connect() as conn:
        ...
        # AUDIT (store a compact summary if available)
        try:
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc010_calcs(user_id: str, limit: int = 200):
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc010_calc(calc_id: str, user_id: str) -> dict | None:
    with connect() as conn:
        r = conn.execute(
            text(f"""
...
def update_dc010_calc(
    ...
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC010").strip() or "DC010"
    ...
    """
    with connect() as conn:
        # Fetch old name for meaningful audit logs
        old = conn.execute(
...
def delete_dc010_calc(calc_id: str, user_id: str) -> bool:
