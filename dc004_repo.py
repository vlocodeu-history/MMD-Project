# dc004_repo.py — with audit logging like dc001_repo.py
...
def create_dc004_calc(user_id: str, name: str, payload: Dict[str, Any]) -> str:
    clean_name = (name or "DC004").strip() or "DC004"
...
def list_dc004_calcs(user_id: str, limit: int = 50) -> List[Tuple[str, str, Any, Any]]:
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc004_calc(calc_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    with connect() as conn:
        row = conn.execute(
...
def update_dc004_calc(
    ...
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC004").strip() or "DC004"
    ...
    with connect() as conn:
        ...
        if ok:
            try:
                # Prefer new name if given; otherwise use the old one
...
def delete_dc004_calc(calc_id: str, user_id: str) -> bool:
    with connect() as conn:
        # Read the name BEFORE deleting, so the audit shows the deleted name
        try:
