# dc007_body_repo.py — with audit logging like dc001_repo.py
...
TABLE  = "dc007_body_calcs"
ENTITY = "dc007_body"

# -------------------------------------------------------------------
# Create
# -------------------------------------------------------------------
def create_dc007_body_calc(
    user_id: str,
    name: str,
    ...
    """
    Insert a DC007-1 (Body) calculation. `payload` is a Python dict (JSONB).
    If `design_id` is provided, the row is linked to a valve design.
    Returns the new row id (as text/uuid).
    """
    clean_name = (name or "DC007-Body").strip() or "DC007-Body"
    sql = f"""
...
# Read (user-scoped)
# -------------------------------------------------------------------
def list_dc007_body_calcs(user_id: str, limit: int = 200):
    with connect() as conn:
        rows = conn.execute(
            text(f"""
...
def get_dc007_body_calc(calc_id: str, user_id: str) -> dict | None:
    with connect() as conn:
        r = conn.execute(
            text(f"""
...
def update_dc007_body_calc(
    ...
    *,
    ...
    if name is not None:
        params["name"] = (name or "DC007-Body").strip() or "DC007-Body"
    ...
    """
    with connect() as conn:
        # Fetch old name first so the audit can always include a name
        old = conn.execute(
...
def delete_dc007_body_calc(calc_id: str, user_id: str) -> bool:
    sql = f"DELETE FROM {TABLE} WHERE id = :id AND user_id = :uid"
    with connect() as conn:
        # Read name BEFORE delete so it appears in the audit entry
        try:
